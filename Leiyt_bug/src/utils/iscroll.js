if(navigator.appVersion == "tianqi") {
    var customUserAgent = "Mozilla/5.0 (iPhone; CPU iPhone OS 11_1_2 like Mac OS X) AppleWebKit/604.3.5 (KHTML, like Gecko) Version/11.0 Mobile/15B202 Safari/604.1"
    var vendor = (/webkit/i).test(customUserAgent) ? 'webkit' : (/firefox/i).test(customUserAgent) ? 'Moz' : 'opera' in window ? 'O' : ''
    var isAndroid = (/android/gi).test(customUserAgent), isIDevice = (/iphone|ipad/gi).test(customUserAgent)
    var isPlaybook = (/playbook/gi).test(customUserAgent), isTouchPad = (/hp-tablet/gi).test(customUserAgent)
}else {
    var vendor = (/webkit/i).test(navigator.appVersion) ? 'webkit' : (/firefox/i).test(navigator.userAgent) ? 'Moz' : 'opera' in window ? 'O' : ''
    var isAndroid = (/android/gi).test(navigator.appVersion), isIDevice = (/iphone|ipad/gi).test(navigator.appVersion)
    var isPlaybook = (/playbook/gi).test(navigator.appVersion), isTouchPad = (/hp-tablet/gi).test(navigator.appVersion)
}
